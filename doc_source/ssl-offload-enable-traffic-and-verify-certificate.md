...
You now have a website that is secured with HTTPS\. The private key for the web server is stored in an HSM in your AWS CloudHSM cluster\. However, you have only one web server\. To set up a second web server and a load balancer for higher availability, go to [Step 5 \(Optional\): Add a load balancer with Elastic Load Balancing](ssl-offload-add-load-balancing.md)\.