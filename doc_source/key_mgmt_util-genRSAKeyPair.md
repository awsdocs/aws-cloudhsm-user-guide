...
The resulting hexadecimal values confirm that it is a public key \(OBJ\_ATTR\_CLASS 0x02\) with a type of RSA \(OBJ\_ATTR\_KEY\_TYPE 0x00\)\. You can use this public key to encrypt \(OBJ\_ATTR\_ENCRYPT 0x01\), but not to decrypt \(OBJ\_ATTR\_DECRYPT 0x00\) or wrap \(OBJ\_ATTR\_WRAP 0x00\)\. The results also include the key length \(512, 0x200\), the modulus, the modulus length \(2048, 0x800\), and the public exponent \(65541, 0x10005\)\.   
